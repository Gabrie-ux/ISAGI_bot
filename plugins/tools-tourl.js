  ...
  try {
    ...
    if (!quoted ||!mime || mime.includes('text/plain') ||!quoted.download) {
      return conn.sendMessage(m.chat, {
        text: `✰ Responde a un archivo con *${usedPrefix + command}*\n\n❀ Soportado: Imágenes, Videos, Audios, Documentos (excepto texto plano)`,
}, { quoted: m})
}
    ...
    if (!media || media.length === 0) throw new Error('No se pudo descargar el archivo')
    if (media.length> 30 * 1024 * 1024) throw new Error('Archivo demasiado grande (máx. 30MB)')
    ...
    if (!ext && mime) {
      ...
      if (fallback && fallback.length <= 4 &&!fallback.includes('-')) ext = fallback
}
    ...
    else if (error.message.includes('large')) msg = 'Archivo demasiado grande (máx. 30MB)'
